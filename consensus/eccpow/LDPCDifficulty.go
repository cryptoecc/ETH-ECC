...
var (
	...
	SensitivityFORKNAME = big.NewInt(2048)
...
func MakeLDPCDifficultyCalculatorFORKNAME() func(time uint64, parent *types.Header) *big.Int {
	return func(time uint64, parent *types.Header) *big.Int {
		bigTime := new(big.Int).SetUint64(time)
		bigParentTime := new(big.Int).SetUint64(parent.Time)
		...
		// parent_diff + (parent_diff / Sensitivity * max((2 if len(parent.uncles) else 1) - ((timestamp - parent.timestamp) // BlockGenerationTime), -99))
		y.Div(parent.Difficulty, SensitivityFORKNAME)
		//fmt.Printf("parent.Difficulty / 8 : %v\n", y)
